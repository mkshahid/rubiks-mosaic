...
#This is the GUI Cube Single-Face Solver
#Last Edited on: 12/5/2014
#Written by: Michael Shahidi (in addition to original cube solver code written by Lucas Liberacki & Tom Brannan)
...
def find_color_middle_piece():
    if a[0][1][1] == desired[4]:
        pass
    elif a[1][1][1] == desired[4]:
        m('X')
    elif a[2][1][1] == desired[4]:
        m('Zi')
    elif a[3][1][1] == desired[4]:
        m('Z')
    elif a[4][1][1] == desired[4]:
        m('Z2')
    elif a[5][1][1] == desired[4]:
        m('Xi')
    else:
        print("Color cannot be found!")
        exit()

def find_color_side_piece(pieceNum):
    # 1. check which center color was inputted and turn face accordingly to make position we are trying to fill top middle
    if pieceNum == 2:
        pass
    elif pieceNum == 4: 
        m('Y')
        ...
        print("Invalid side piece number entry!")
        quit()
    # 2. check if already in that spot
    if a[0][0][1] == desired[pieceNum - 1]:
        pass
    # 3. look for colors in different positions of the cube in order of positions that require the least amount of moves (perform adjacent move last)
    # Require 1 move
    elif a[2][0][1] == desired[pieceNum - 1]:
        m('B')
    elif a[3][0][1] == desired[pieceNum - 1]:
        m('Bi')
    # Require 2 moves 
    elif a[1][1][2] == desired[pieceNum - 1]:
        m('DW B')
    elif a[1][1][0] == desired[pieceNum - 1]:
        m('DWi Bi')
    elif a[4][2][1] == desired[pieceNum - 1]:
        m('B2')
    elif a[5][1][0] == desired[pieceNum - 1]:
        m('DWi B')
    elif a[5][1][2] == desired[pieceNum - 1]:
        m('DW Bi') 
    # Require 3 moves  
    elif a[2][2][1] == desired[pieceNum - 1]:
        m('DW2 Bi')
    elif a[2][1][2] == desired[pieceNum - 1]:
        m('Ri B R')
    elif a[3][2][1] == desired[pieceNum - 1]:
        m('DW2 B')
    elif a[3][1][0] == desired[pieceNum - 1]:
        m('L Bi Li')
    elif a[4][1][2] == desired[pieceNum - 1]:
        m('D B2')
    elif a[4][1][0] == desired[pieceNum - 1]:
        m('Di B2')
    # Require 4 moves   
    elif a[1][2][1] == desired[pieceNum - 1]:
        m('D Ri B R')
    elif a[4][0][1] == desired[pieceNum - 1]:
        m('D2 B2')
    elif a[5][0][1] == desired[pieceNum - 1]:
        m('Di Ri B R')
    # Adjacent position
    elif a[5][2][1] == desired[pieceNum - 1]:
        m('Bi DW Bi')
    # No colors in above positions found (must be an error)  
    else:
        print("Cannot find color!")
        quit()
    # Revert the orientation of the cube back to original
    if pieceNum == 4: 
        m('Yi')
    elif pieceNum == 8:
        m('Y2')
...
def find_color_corner_piece(pieceNum):
    # 1. check which center color was inputted and turn face accordingly to make position we are trying to fill top right corner
    if pieceNum == 3:
        pass
    elif pieceNum == 1: 
        m('Y')
        ...
        m('Yi')
    # 2. check if already in that spot
    if a[0][0][2] == desired[pieceNum - 1]:
        pass
    # 3. look for colors in different positions of the cube in order of positions that require the least amount of moves (perform adjacent move last)
    # Requires 3 moves
    elif a[1][2][2] == desired[pieceNum - 1]:
        m('Bi D B')
    elif a[2][0][2] == desired[pieceNum - 1]:
        m('LW B LWi')
    elif a[5][0][2] == desired[pieceNum - 1]:
        m('Bi Di B')
    # Requires 4 moves
    elif a[2][2][2] == desired[pieceNum - 1]:
        m('D Bi Di B')
    elif a[3][0][0] == desired[pieceNum - 1]:
        m('Di Bi Di B')
    elif a[3][2][0] == desired[pieceNum - 1]:
        m('D Bi D B')
    elif a[5][0][0] == desired[pieceNum - 1]:
        m('D2 Bi D B')
    # Requires 5 moves
    elif a[1][2][0] == desired[pieceNum - 1]:
        m('D2 Bi Di B')
    # Requires 8 moves
    elif a[4][2][2] == desired[pieceNum - 1]:
        m('Bi D2 B D Bi Di B')
    # Requires 9 moves
    elif a[4][0][2] == desired[pieceNum - 1]:
        m('D Bi D2 B D Bi Di B')
    elif a[4][2][0] == desired[pieceNum - 1]:
        m('Di Bi D2 B D Bi Di B')
    # Requires 10 moves
    elif a[4][0][0] == desired[pieceNum - 1]:
        m('D2 Bi D2 B D Bi Di B')
    # Adjacent positions
    elif a[2][0][0] == desired[pieceNum - 1]: 
        m('Bi D B Di Bi D B')
    elif a[5][2][2] == desired[pieceNum - 1]: 
        while a[0][0][2] != desired[pieceNum - 1]: 
            m('Bi Di B D')
    # No colors in above positions found (must be an error)
    else:
        print("Cannot find color!")
...
def isSolved():
    # THIS SHOULD JUST BE USIDE
    uside = a[0][0][0] == desired[0] and a[0][0][1] == desired[1] and a[0][0][2] == desired[2] and a[0][1][0] == desired[3] and a[0][1][1] == desired[4] and a[0][1][2] == desired[5] and a[0][2][0] == desired[6] and a[0][2][1] == desired[7] and a[0][2][2] == desired[8]
    fside = a[1][0][0] == desired[0] and a[1][0][1] == desired[1] and a[1][0][2] == desired[2] and a[1][1][0] == desired[3] and a[1][1][1] == desired[4] and a[1][1][2] == desired[5] and a[1][2][0] == desired[6] and a[1][2][1] == desired[7] and a[1][2][2] == desired[8]
    rside = a[2][0][0] == desired[0] and a[2][0][1] == desired[1] and a[2][0][2] == desired[2] and a[2][1][0] == desired[3] and a[2][1][1] == desired[4] and a[2][1][2] == desired[5] and a[2][2][0] == desired[6] and a[2][2][1] == desired[7] and a[2][2][2] == desired[8]
    lside = a[3][0][0] == desired[0] and a[3][0][1] == desired[1] and a[3][0][2] == desired[2] and a[3][1][0] == desired[3] and a[3][1][1] == desired[4] and a[3][1][2] == desired[5] and a[3][2][0] == desired[6] and a[3][2][1] == desired[7] and a[3][2][2] == desired[8]
    dside = a[4][0][0] == desired[0] and a[4][0][1] == desired[1] and a[4][0][2] == desired[2] and a[4][1][0] == desired[3] and a[4][1][1] == desired[4] and a[4][1][2] == desired[5] and a[4][2][0] == desired[6] and a[4][2][1] == desired[7] and a[4][2][2] == desired[8]
    bside = a[5][0][0] == desired[0] and a[5][0][1] == desired[1] and a[5][0][2] == desired[2] and a[5][1][0] == desired[3] and a[5][1][1] == desired[4] and a[5][1][2] == desired[5] and a[5][2][0] == desired[6] and a[5][2][1] == desired[7] and a[5][2][2] == desired[8]
    return uside or fside or rside or lside or dside or bside

def solve():
    find_color_middle_piece()
    print_cube()
    print("5")
    find_color_side_piece(2)
    simplify_moves()
    print_cube()
    print("2")
    find_color_side_piece(6)
    simplify_moves()
    print_cube()
    print("6")
    find_color_side_piece(8)
    simplify_moves()
    print_cube()
    print("8")
    find_color_side_piece(4)
    simplify_moves()
    print_cube()
    print("4")
    find_color_corner_piece(3)
    simplify_moves()
    print_cube()
    print("3")
    find_color_corner_piece(9)
    simplify_moves()
    print_cube()
    print("9")
    find_color_corner_piece(7)
    simplify_moves()
    print_cube()
    print("7")
    find_color_corner_piece(1)
    simplify_moves()
    print_cube()
    print("1")
    print_cube()
...
#GUIsetDesiredOutput is used to set the desired output of the cube
def GUIsetDesiredOutput(desiredOutput):
    global desired, DesiredOutputDisplay
    if len(desiredOutput.get()) == 9:
        desired = [desiredOutput.get()[0],desiredOutput.get()[1],desiredOutput.get()[2],
                desiredOutput.get()[3],desiredOutput.get()[4],desiredOutput.get()[5],
                desiredOutput.get()[6],desiredOutput.get()[7],desiredOutput.get()[8]]
        DesiredOutputDisplay.configure(text="Output: " + desired[0] + desired[1] + desired[2] + desired[3] + desired[4] + desired[5] + desired[6] + desired[7] + desired[8])
        return
    print("desired set to WWWWWWWWW")
    desired = ['W','W','W',
               'W','W','W',
               'W','W','W']
    DesiredOutputDisplay.configure(text="Output: " + desired[0] + desired[1] + desired[2] + desired[3] + desired[4] + desired[5] + desired[6] + desired[7] + desired[8])
...
def InfoGUI():
    rt = tk.Tk()
    rt.geometry("600x135+50+50") #size of the starting frame
...
def GUI():
    global root
    root = tk.Tk()
    root.geometry("650x500+50+50") #size of the starting frame
...
def GUIy():
    global root, canvas, ScrambleLabel, SolutionLabel, SolutionNumberLabel, frame,isTransparent, DesiredOutputDisplay
    ...
    # row 1.5 - label to tell you to enter desired output
    DesiredOutput = Label(frame, text ="Enter desired output:").grid(row = 2,column = 0)
    DesiredOutputDisplay = Label(frame, text ="Output: " + desired[0] + desired[1] + desired[2] + desired[3] + desired[4] + desired[5] + desired[6] + desired[7] + desired[8])
    DesiredOutputDisplay.grid(row = 2, column=1)
    print(DesiredOutputDisplay)
    DesiredOutputButton=Button(frame,text="Execute", command = lambda: GUIsetDesiredOutput(desiredOutput)).grid(row = 3,column = 1, sticky="w")
    # row 1.75 - has entry for desired output 
    DesiredOutputEntry = Entry(frame, textvariable=desiredOutput).grid(row = 3, column=0)
    #row 2 - label to tell you to enter a move for execution
    EnterMove = Label(frame, text ="Enter move(s):").grid(row=4,column=0)
    #row 3 - Has entry for custom moves as well as button to execute them
    MoveEntry = Entry(frame, textvariable=move).grid(row = 5, column=0)
    DrawCubeButton=Button(frame,text="Execute", command = lambda: GUImakeMove(move)).grid(row = 5,column = 1, sticky="w")
    #row 4 - The label that will print out the current scramble after generation
    ScrambleLabel = Label(frame, text="Scramble will be displayed here",wraplength=250, justify=CENTER, height = 2)
    ScrambleLabel.grid(row=6,column=0, columnspan=2)
    #row 5 - The scramble button to generate new scramble and copy scramble to clipboard
    ScrambleButton = Button(frame, text="Scramble",bg="lightgreen", command = lambda: GUIScramble()).grid(row = 7, column = 0)
    CopyScrambleButton = Button(frame, text="Copy Scramble",bg="#EF9", command = lambda: GUItoClipboard(get_scramble())).grid(row = 7, column = 1)
    ...
    SolveButton = Button(frame, text="Solve Cube",bg="#D53",command = lambda: GUISolve()).grid(row = 9, column = 0) #sticky="e" if using timer button as well
    CopyScrambleButton = Button(frame, text="Copy Solution",bg="#EF9", command = lambda: GUItoClipboard(get_moves())).grid(row = 9, column = 1)
